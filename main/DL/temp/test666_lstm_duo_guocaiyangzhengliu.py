...
# # 训练函数 网络，训练迭代器，验证迭代器，epoch，学习率，权重衰减
...
#             features = reshape_for_lstm(features)  # 重新调整输入数据的形状
#             trainer.zero_grad()
#             pred = net(features)
#             l = loss_fn(pred, labels)
#             l.sum().backward()
#             trainer.step()  # 反向传播更新权重
...
#                     X = X.to(device)
#                     targets = y.to(device).long()  # 标签转为long类型用于CrossEntropyLoss
...
#                     os.mkdir('./model')
#                 torch.save(net.state_dict(), './model/best_model_lstm.pth')
...
if __name__ == '__main__':
    num_epochs, lr, wd, batch_size, alpha, temperature = 20, 8e-5, 6e-6, 256, 0.5, 2.0
    # 1. 载入数据集和预处理
    ...
    train_features = torch.tensor(train_features[:n_train].values, dtype=torch.float32)
    # 训练和测试标签转换为一维long类型
    train_labels = torch.tensor(train_labels.values, dtype=torch.long)  # 一维的long类型标签
    ...
    # 7. 定义和训练模型
    teacher_model = TeacherModel(input_size=feature_type_count, num_classes=num_classes)  # 定义教师模型
    student_model = StudentModel(input_size=feature_type_count, num_classes=num_classes)  # 定义学生模型
    # 6. 调用训练函数，训练学生模型
    train_with_distillation(student_model, teacher_model, train_iter, valid_iter, num_epochs, lr, wd, len(valid_labels),
                            alpha, temperature)

    # 9. 评估学生模型
    test_model(test_iter, student_model)
