...
def load_label_mapping(file_path='label_mapping.txt'):
    label_mapping = {}
...
def test_model(test_iter, model,  label_mapping_path='label_mapping.txt'):
    model.eval()  # 切换到评估模式
    correct = 0
    total = 0
    label_counts = {label: 0 for label in load_label_mapping(label_mapping_path).values()}  # 统计每个类别的数量
    ...
    with torch.no_grad():
        for inputs, labels in test_iter:
            ...
            for label in predicted.tolist():  # 使用 tolist() 将 tensor 转换为列表
                predicted_label = label_mapping[label]  # 获取对应的标签名
                label_counts[predicted_label] += 1

            predicted = predicted.view(-1, 1)  # 或者使用 predicted.unsqueeze(1)
...
if __name__ == '__main__':
    ...
    student_net = StudentModel(num_classes=num_classes)
