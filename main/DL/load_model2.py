...
def preprocess_data(train_df, test_df, overSamplingValue):
    ...
    train_df = pd.get_dummies(train_df, columns=categorical_columns, drop_first=True, dtype=int)
    test_df = pd.get_dummies(test_df, columns=categorical_columns, drop_first=True, dtype=int)
    ...
    # 6. 标准化：使用训练集的均值和标准差对训练集和测试集进行标准化
...
def main(num_epochs=20, lr=5e-6, wd=6e-6, batch_size=256, alpha=0.5, temperature=2.0,overSamplingValue=0):
    ...
    train_features, train_labels, test_features, test_labels = preprocess_data(data_train_df, data_test_df,overSamplingValue)
